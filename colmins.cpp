...
// [[Rcpp::export]]
DataFrame colmins(DataFrame df) {
  ...
  for (int i = 0; i < ncols; i++) {
    ...
    case REALSXP: {
      ...
      double min_val = NA_REAL;
      for (int j = 0; j < n; j++) {
        if (!NumericVector::is_na(num_col[j])) {
          min_val = num_col[j];
          // Continue from next element to find actual minimum
          for (int k = j + 1; k < n; k++) {
            if (!NumericVector::is_na(num_col[k]) && num_col[k] < min_val) {
              min_val = num_col[k];
            }
          }
          break;
      ...
      result_list[i] = NumericVector::create(min_val);
      break;
    }
    ...
    case INTSXP: {
      ...
      int min_val = NA_INTEGER;
      for (int j = 0; j < n; j++) {
        if (!IntegerVector::is_na(int_col[j])) {
          min_val = int_col[j];
          for (int k = j + 1; k < n; k++) {
            if (!IntegerVector::is_na(int_col[k]) && int_col[k] < min_val) {
              min_val = int_col[k];
            }
          }
          break;
      ...
      result_list[i] = IntegerVector::create(min_val);
      break;
    }
    ...
    case LGLSXP: {
      ...
      // Optimized logical comparison: FALSE < TRUE, so early exit possible
      int min_val = NA_LOGICAL;
      for (int j = 0; j < n; j++) {
        if (!LogicalVector::is_na(log_col[j])) {
          min_val = log_col[j];
          if (min_val == FALSE) {
            break; // FALSE is minimum possible logical value
          }
          // Continue looking for FALSE
          for (int k = j + 1; k < n; k++) {
            if (!LogicalVector::is_na(log_col[k]) && log_col[k] < min_val) {
              min_val = log_col[k];
              if (min_val == FALSE) break; // Early exit optimization
            }
          }
          break;
        }
      }
      result_list[i] = LogicalVector::create(min_val);
      break;
    }
    ...
    case STRSXP: {
      ...
      String min_val = NA_STRING;
      for (int j = 0; j < n; j++) {
        if (!CharacterVector::is_na(char_col[j])) {
          min_val = String(char_col[j]);  // Explicit conversion
          for (int k = j + 1; k < n; k++) {
      ...
      result_list[i] = CharacterVector::create(min_val);
      break;
    }
